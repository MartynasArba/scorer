...
class HeuristicScorer2():
    """
    Heuristic-based scorer class
    Different logic than v1
    ...
    def score(self):
        ...
        for window_id in tqdm(range(self.data.all_samples.size(dim = 0))):
            ...
            win_vals = window.median(dim = -1).values
            #first, checking by EMG power, channel 3
            if (win_vals[3] >= self.q90[3]).item():
                state = 1   #W
            # checking REM - high theta, low emg, could add low delta?
            elif (win_vals[5] >= self.q50[5]).item() and (win_vals[3] <= self.q50[3]).item() and (win_vals[4] <= self.q50[4]).item():
                if window_id != 0:
                    if self.states[window_id-1] != 1:#previous state can't be W
                        state = 4 #REM
            #checking for IS - high sigma, sigma > theta, low emg
            elif (win_vals[7] > self.q50[7]).item() and (win_vals[7] > win_vals[5]).item() and (win_vals[3] <= self.q50[3]).item():
                state = 3
            #NREM: high delta, low emg
            elif (win_vals[4] > self.q50[4]).item() and (win_vals[3] <= self.q50[3]).item():
                state = 2
    ...
    def _get_qs(self, data: SleepSignals, sub_factor: int = 10):
        ...
        sub = data.all_samples[::sub_factor, :, :].mean(dim=-1).float()
        
        #for each channel, select a subset and calculate summary stats
        for ch in range(n_channels):
            q10.append(torch.quantile(sub[:, ch], q = 0.10))#"low"
            q50.append(torch.quantile(sub[:, ch], q = 0.50))#"mid"
            q90.append(torch.quantile(sub[:, ch], q = 0.90))#'high"
        return torch.stack(q10), torch.stack(q50), torch.stack(q90)
