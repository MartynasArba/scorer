...
class MockSequenceDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, seq_len=10, n_samples=20, channels=2, win_len=1000, device='cpu'):
        self.seq_len = seq_len
        self.n_samples = n_samples
        self.device = device
        # [Batch, Seq, Chan, Time] - Use 2 channels to match real data flow
...
class TestSequenceTraining(unittest.TestCase):
    def setUp(self):
        ...
        self.input_channels = 2 # Match the actual data flow (ECoG and EMG)
        self.win_len = 1000
